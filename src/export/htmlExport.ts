 ...
 */
function capturePagesContent(selector: string): string {
    const pagesContent = document.querySelector(selector);

    if (!pagesContent) {
        console.error(`[Export] Could not find ${selector} element`);
...
export async function exportToHTML(
    ...
    const {
        ...
        title = 'Canvas Export',
        contentSelector = '.pages-content',
        generatorName = 'DungeonMind Canvas',
    } = options;

    const displayName = adapters.metadataExtractor.extractDisplayName(page.dataSources) || 'Document';
    ...
    <meta name="document" content="${displayName}">
    ...
    const pagesContent = capturePagesContent(contentSelector);
...
export async function exportPageToHTMLFile(
    ...
    const displayName = adapters.metadataExtractor.extractDisplayName(page.dataSources) || 'document';
    const filename = `${displayName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_export.html`;
